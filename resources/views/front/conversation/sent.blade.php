                                        ...
                                        <div class="mailbox-menu">
                                            <ul class="mailmenu-list-unstyled">
                                                <li><a href="{{ url('/conversation') }}"  >Inbox <span
                                                            class="mail-index"></span></a>
                                                </li>
                                                <li><a href="{{ url('/conversation-sent') }} "class="active">Sent</a></li>
                                                ...
                                                @foreach ($conversations as $conversation)
                                                    ...
                                                    <div class="mail-info">
                                                        <div class="mail-list-items">
                                                            {{-- <div class="mail-list-items @if($user->user_id != $globaldata['user']->user_id){{(!empty($messageread) && $messageread->is_read == 1?'read':'unread')}}@endif"> --}}
                                                                <a href="{{ url('/conversation-details/'. $conversation->conversation_id) }}" class="mail-author-info">
                                                                    <img src="{{$conversation->touser->imagefilepath}}" alt="mail author image"
                                                                        class="mail-author-image">
                                                                        <span class="mail-author-name">
                                                                            {{$conversation->touser->username}}
                                                                        </span>
                                                                        <span>{{ Carbon\Carbon::parse($conversation->touser->dob)->age }}{{ $conversation->touser->gender }}</span>
                                                                        <p>{{string_explode_implode($conversation->touser->address)}}</p>
